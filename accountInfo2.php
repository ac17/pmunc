...
if (isset($_POST['editSubmit']) && $_POST['editSubmit'] == 'true') {
	...
	if ($row['PC'] == 'TRUE')
		$query2 = 'UPDATE SCHOOL_INFO set HD_Email="' . $newEmail . '", Preferred_Email="' . $newEmail . '" WHERE HD_Email="' . $oldEmail . '";';
	else
		$query2 = 'UPDATE SCHOOL_INFO set FA_Email="' . $newEmail . '", Preferred_Email="' . $newEmail . '" WHERE FA_Email="' . $oldEmail . '";';
	if (!$errors) {
		if (mysql_query($query1) && mysql_query($query2)) {
			$success['editAccountInfo'] = 'Your information has been updated.';
			$query3 = 'SELECT * FROM users WHERE email="' . $newEmail . '";';
			$_SESSION['user'] = mysql_fetch_assoc(mysql_query($query3));
		}
		else {
			$errors['editAccountInfo'] = 'There was a problem updating your information. Please try again.';
		}
	}
}
...
if (isset($_POST['registerSubmit']) && $_POST['registerSubmit'] == 'true') {
	...
	. '", Share="' . $share . '" WHERE Preferred_Email="' . $_SESSION['user']['email'] . '";';

	$query2 = 'UPDATE users set email="' . $preferredEmail . '", first_name="' . $preferredContact
	. '" WHERE email="' . $_SESSION['user']['email'] . '";';

	if(mysql_query($query1) && mysql_query($query2)) {
		$success['register'] = 'Your delegation information has been updated.';
		$query3 = 'SELECT * FROM users WHERE email="' . $preferredEmail . '";';
		$_SESSION['user'] = mysql_fetch_assoc(mysql_query($query3));
	}
	else {
		$errors['register'] = 'There was a problem updating your information. Please check your details and try again.';
	}
}
?>
<!DOCTYPE html>
  ...
  ga('send', 'pageview');
	...
	<body>
		...
		<div class="container" id="template-container">
			<div class="container">
				<h1 class="text-center">Edit Your Account Information</h1>
				...
				<form name="editForm" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
					...
					?>
					<h3><b>New email</b>: <input type="email" name="email" class="form-control" value="<?php echo $_SESSION['user']['email']; ?>" required><br>
					<h3><b>New password</b>: <input type="password" name="password" value="" class="form-control" placeholder="Password" required><br>
					<h3><b>Confirm new password</b>: <input type="password" name="confirmPassword" value="" class="form-control" placeholder="Password" required><br>
					<?php if ($errors['newConfirmPassword']) print "<div class=\"invalid\">" . $errors['newConfirmPassword'] . "</div>"; ?></h3>
					<input type="hidden" name="editSubmit" id="editSubmit" value="true">
					<input class="btn btn-lg btn-primary btn-block" type="submit" value="Submit">
				</form><br>
				<h1 class="text-center">Edit Your Delegation Information</h1>
				<div class="horbar"></div><br>
