...
$index = (int) $_GET["committee"];
$multipleChairEmails = array("ebackman@princeton.edu", "lsaud@princeton.edu",
	"naomilee@princeton.edu");

$query0 = "SELECT * FROM COMMITTEE_INFO WHERE Name='" . mysql_real_escape_string($committees[$index]) . "';";
$result=mysql_query($query0);
if (mysql_num_rows($result) > 1) {
	if ($index >= count($committees)-3)
		$result = mysql_query('SELECT * FROM COMMITTEE_INFO WHERE Email="'
			. $multipleChairEmails[$index - count($committees) + 3] . '";');
}
...
if (isset($_POST['editSubmit']) && $_POST['editSubmit'] == 'true') {
	...
	if (strlen($newDesc) > 20000)
		$errors['newDesc'] = 'Your topic description cannot be longer than 20,000 characters.';
...
if (empty($_FILES["committeeApp"]["name"])) {
	...
	$query1 = 'UPDATE COMMITTEE_INFO SET description="' . $newDesc . '", BG_Filename="' . $BG . '", A_Filename="' . $CA . '" WHERE Name="' . mysql_real_escape_string($committees[$index]) . '";';
	$query2 = 'UPDATE users SET email = "' . $newEmail . '" WHERE email="' . $oldEmail . '";';
	$query3 = 'UPDATE COMMITTEE_INFO SET Leader="' . $newChair . '", Email="' . $newEmail . '", bio="' . $newBio . '", P_Filename="' . $profFile . '" WHERE Email="' . $oldEmail . '";';
	if (!$errors) {
        if (mysql_query($query1) && mysql_query($query2) && mysql_query($query3)) {
            $success['editCommitteeInfo'] = 'The committee information was updated.' . $oldEmail;
            $row = mysql_fetch_assoc(mysql_query('SELECT * FROM COMMITTEE_INFO WHERE Email="' . $newEmail . '";'));
        }
        else {
	...
	<body>
		...
		<div class="container" id="template-container">
			<div class="container">
				...
				<h3><b>Chair Topic Description</b>:</h3> <?php echo $row['description']; ?>
				<h3><b>Chair Photo</b>:</h3>
				<?php
				if ($row['P_Filename'] == NULL) echo "<p><i>No image has been uploaded.</i></p>"; 
				else echo "<img src=\"" . $row['P_Filename'] . "\" width=\"25%\">";
				?>
				<h3><b>Background Guide</b>:</h3>
					<?php
				...
				<h3><b>Committee Application</b>:</h3>
				<?php
				...
				<form name="editForm" role="form" action="<?php echo 'committeeEdit1.php?committee=' . $index; ?>" enctype="multipart/form-data" method="post">
					...
					<textarea width=100% name="topic" class="form-control"><?php echo $row['description']; ?></textarea>
