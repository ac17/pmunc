...
$success=array();
	...
	$msg = trim($_POST['msg']) . "\n\n Message sent by " . $_SESSION['user']['first_name'] . " (" . $_SESSION['user']['email'] . ").";
	$mail = mail("pmunc@princeton.edu", trim($_POST['subj']), $msg);

	if ($mail) {
		$success['sendEmail'] = 'Your email has been sent. Thank you for your message! We will get back to you as soon as possible.';
		$pageTitle = 'Email Sent';
	}
	else {
		$errors['sendEmail'] = 'There was a problem with sending your email. Please try again.';
		$pageTitle = 'Email Delivery Failure';
	}
			...
			<div class="container">
				...
				<center>
					...
					if ($success['sendEmail'])
						echo $success['sendEmail'];
					else
						echo $errors['sendEmail'];
					?>
				</center>
			</div>
		</div>
