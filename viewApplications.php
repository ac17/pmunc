...
$apps = mysql_query('SELECT * FROM APPLICATIONS WHERE Committee="' . $committees[$index] . '" ORDER BY School, Name ASC;');
?>
<!DOCTYPE html>
<html lang="en">
	<head>
  ...
  ga('send', 'pageview');
	...
	<body>
		...
		<div class="container" id="template-container">
			<div class="container">
				<h1 class="text-center">Submitted Committee Applications</h1>
				<div class="horbar">
				</div><br>
				<h2 class="text-center"><i><?php echo $committees[$index]; ?></i></h2>
				<?php
				echo "<table class=\"table\">
				<thead><tr><th>Applicant</th>
				<th>School</th>
				<th>Download Application</th>
				</tr></thead>
				<tbody>";
				for ($i=0; $i < mysql_num_rows($apps); $i++) {
					$row = mysql_fetch_assoc($apps);
					echo "<tr><td>" . $row['Name'] . "</td><td>" . $row['School'] 
					. "</td><td><a href=\"" . $row['Filename'] . "\">Download</a></td></tr>";
				}
				echo "</tbody></table>";
				?>
				<center>
					<a href="committeeManagement1.php"><button type="button" class="btn btn-link">Back to Committee Management</button></a><br>
