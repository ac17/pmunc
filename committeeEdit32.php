...
if (empty($_FILES["backgroundGuide"]["name"])) {
	...
	$query1 = 'UPDATE COMMITTEE_INFO SET bio="' . $newBio . '", P_Filename="' . $profFile . '"  WHERE Email="' . $oldEmail . '";';
	$query2 = 'UPDATE COMMITTEE_INFO SET description="' . $newDesc . '", BG_Filename="' . $BG . '" WHERE Name="' . $row['Name'] . '";';
	if (!$errors) {
