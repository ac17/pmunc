...
if (isset($_POST['editSubmit']) && $_POST['editSubmit'] == 'true') {
	...
	if (!$errors) {
		if (mysql_query($query1) && mysql_query($query2)) {
			...
			$userRow = mysql_fetch_assoc(mysql_query('SELECT * FROM users WHERE session_id="' . session_id() . '";'));
	...
	<body>
		...
		<div class="container" id="template-container">
			<div class="container">
				...
				<h3><b>Email</b>: <?php echo $_SESSION['user']['email']; ?></h3>
				<h3><b>Password</b>: <i>(Not displayed for security reasons.)</i></h3><br>
				...
				<form name="editForm" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
					...
					<?php if ($errors['newEmail']) print "<div class=\"invalid\">" . $errors['newEmail'] . "</div>"; ?></h3>
					<h3><b>New password</b>: <input type="password" name="password" value="" class="form-control" placeholder="Password" required><br>
					<?php if ($errors['newPassword']) print "<div class=\"invalid\">" . $errors['newPassword'] . "</div>"; ?></h3>
