  ...
  ga('send', 'pageview');
	...
	<body>
		...
		<div class="container" id="template-container">
			<div class="container">
				<h1 class="text-center">View Applications</h1>
				...
				/* get applications */
				$apps = mysql_query('SELECT * FROM APPLICATIONS WHERE Committee="' . $committeeInfo['Name'] . '" ORDER BY Name ASC;');
				if ($apps) {
					echo "<table class=\"table\"><thead><tr><th>Applicant</th><th>Email</th><th>School</th><th>Download Link</th></thead><tbody>";
					for ($x=0;$x<mysql_num_rows($apps);$x++) {
						$appInfo = mysql_fetch_assoc($apps);
						echo "<tr><td>" . $appInfo['Name'] . "</td><td>" . $appInfo['Email'] . "</td><td>" . $appInfo['School'] . "</td><td><a href=\"" . $appInfo['Filename'] . "\">Download</a></td></tr>";
					}
					echo "</tbody></table>";
				}
				?>
