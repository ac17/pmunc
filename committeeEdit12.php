...
if (mysql_num_rows($result) > 1) {
	if ($index == count($committees)-1)
		$result = mysql_query('SELECT * FROM COMMITTEE_INFO WHERE Email="lsaud@princeton.edu";');
}
...
if (empty($_FILES["backgroundGuide"]["name"])) {
	...
	$query1 = 'UPDATE COMMITTEE_INFO SET description="' . $newDesc . '", BG_Filename="' . $BG . '" WHERE Name="' . $committees[$index] . '";';
	...
	if (!$errors) {
        if (mysql_query($query1) && mysql_query($query2) && mysql_query($query3)) {
            ...
            $errors['editCommmitteeInfo'] = 'There was a problem updating the information. Please try again.';
        }
    }
}
?>
		...
		<div class="container" id="template-container">
			<div class="container">
				...
				<h3><b>Background Guide</b>:</h3>
					<?php
					if ($row['BG_Filename'] == NULL) echo "<p><i>No background guide has been uploaded.</i></p><br>";
					else echo "<a href=\"" . $row['BG_Filename'] . "\">Download Here</a>";
					?>
				</h3>
				<h1 class="text-center">Edit Committee Info</h1>
				<div class="horbar"></div><br>
				<form name="editForm" role="form" action="<?php echo 'committeeEdit12.php?committee=' . $index; ?>" enctype="multipart/form-data" method="post">
                    <?php
